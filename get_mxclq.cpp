...
int GET::none_degree[tab_node_size];
...
GET::GET(){
    ...
    APPEND_STACK_SIZE = 0; APPEND_STACK_USED = 0;
    ...
    memset(iSET_Size, 0, sizeof(iSET_Size));
    memset(iSET_State, 0, sizeof(iSET_State));
    memset(iSET_Used, 0, sizeof(iSET_Used));
    memset(iSET_Tested, 0, sizeof(iSET_Tested));
    memset(iSET_Index, 0, sizeof(iSET_Index));
    memset(active_degree, 0, sizeof(active_degree));
    memset(Clique_Stack, 0, sizeof(Clique_Stack));
    memset(MaxCLQ_Stack, 0, sizeof(MaxCLQ_Stack));
    memset(Candidate_Stack, 0, sizeof(Candidate_Stack));
    memset(Cursor_Stack, 0, sizeof(Cursor_Stack));
    memset(Vertex_UB, 0, sizeof(Vertex_UB));
    memset(NEW_OLD, 0, sizeof(NEW_OLD));
    memset(OLD_NEW, 0, sizeof(OLD_NEW));
    memset(Extra_Node_Stack, 0, sizeof(Extra_Node_Stack));
    memset(Branches, 0, sizeof(Branches));
    memset(iSET, 0, sizeof(iSET));
    memset(ADDED_NODE_iSET, 0, sizeof(ADDED_NODE_iSET));
    memset(iSET_Involved, 0, sizeof(iSET_Involved));
	...
	flag = nullptr;
	flag = (int**)malloc(tab_node_size * sizeof(int *));
	for(int i = 0; i < tab_node_size; i++){
		flag[i] = nullptr;
	}
    for (int i = 0; i < tab_node_size; ++i) {
        node_neibors[i] = nullptr;
    ...
    memset(FIXED_NODE_STACK, 0, sizeof(FIXED_NODE_STACK));
    memset(UNIT_STACK, 0, sizeof(UNIT_STACK));
    memset(NEW_UNIT_STACK, 0, sizeof(NEW_UNIT_STACK));
}
...
GET::~GET(){
    ...
    if (INIT_Stack != nullptr) {
        free(INIT_Stack);
        INIT_Stack = nullptr;
    }
    if (APPEND_STACK != nullptr) {
        free(APPEND_STACK);
        APPEND_STACK = nullptr;
    }
}

int GET::none_degree_inc(const void * a, const void * b) {
...
int GET::static_degree_dec(const void * a, const void * b) {
	int node1 = *((int *) a), node2 = *((int *) b);
...
float GET::density(int nb_node,int nb_edge){
	if(nb_node <=1 || nb_edge==0)
		return 0;
	else
		return 2*((float)nb_edge)/((nb_node)*(nb_node-1));
}

int GET::build_simple_graph_instance(EdgeListGraph& graph) {
	...
	for(int i = 0; i < NB_NODE; i++){
		for(int j = graph.pstart[i]; j < graph.pstart[i + 1]; j++){
			...
			if(i >= v)
				continue;
			if(matrice[i + 1][v + 1] == FALSE){
				matrice[i + 1][v + 1] = TRUE;
				matrice[v + 1][i + 1] = TRUE;
				static_degree[i + 1]++;
				static_degree[v + 1]++;
				nb_edge++;
			}
	...
	for (i = 1; i <= NB_NODE; i++) {
		...
		none_neibors[i] = (int *) malloc((NB_NODE+2) * sizeof(int));
        flag[i] = none_neibors[i];
...
void GET::search_initial_maximum_clique() {
...
void GET::sort_by_active_degree() {
...
void GET::my_sort_by_degree_dec() {
...
void GET::my_sort_by_degree_inc() {
...
int GET::iset_smaller_than(int iset1, int iset2) {
...
void GET::sort_isets_and_push_nodes() {
...
void GET::print_isets(int i_set) {
	int *color_set, node;
...
int GET::addIntoIsetTomitaBis(int node) {
...
int GET::cut_by_iset_last_renumber() {
...
int GET::fix_newNode_for_iset(int fix_node, int fix_iset) {
	int idx, iset_idx;
...
int GET::fix_oldNode_for_iset(int fix_node, int fix_iset) {
...
int GET::fix_node_iset(int fix_iset) {
...
int GET::unit_iset_process_used_first() {
...
void GET::rollback_context_for_maxsatz(int start_fixed, int start_passive,
		int start_reduced) {
	int i, node;
...
void GET::reset_context_for_maxsatz() {
	int i, node;
...
int GET::further_test_reduced_iset(int start) {
...
int GET::fix_anyNode_for_iset(int fix_node, int fix_iset) {
	if (fix_node > NB_NODE)
		return fix_newNode_for_iset(fix_node, fix_iset);
	else
		return fix_oldNode_for_iset(fix_node, fix_iset);
...
int GET::inc_maxsatz_lookahead_by_fl2() {
...
int GET::inc_maxsatz_on_last_iset(int end) {
...
int GET::open_new_iset_old(int i) {
...
int GET::simple_further_test_node(int start) {
...
int GET::test_node_for_failed_nodes(int node, int iset) {
	int my_iset, saved_node_stack_fill_pointer,
			saved_passive_iset_stack_fill_pointer,
			saved_reduced_iset_stack_fill_pointer;
...
int GET::test_by_eliminate_failed_nodes() {
...
int GET::cut_by_inc_maxsat_eliminate_first() {
	int i, j, k = 0, node;
...
void GET::rebuild_matrix(int start) {
...
void GET::store_maximal_clique(int node) {
...
void GET::store_maximum_clique(int node, int print_info) {
	int i = 0;
	if (REBUILD_MATRIX == TRUE && CUR_CLQ_SIZE > 0)
		push(OLD_NEW[node], Clique_Stack);
	else
...
int GET::cut_by_inc_ub(int print_info) {
...
void GET::search_maxclique(int cutoff, int print_info) {
	int node;
	init_for_search();
	BRANCHING_COUNT = 0;
	if (print_info == TRUE) {
...
int GET::sort_by_maxiset(int mandatory) {
...
void GET::init_for_maxclique(int ordering, int list_all) {
	if(NB_NODE<=1)
		DENSITY = 0;
	else
...
std::vector<int> GET::get_mxclq(EdgeListGraph& graph) {
	long sec, usec, sec_p, usec_p;
	int i, ordering = -1, _all = FALSE;
	INIT_CLIQUE = 0;
	LIST_ALL = FALSE;

	build_simple_graph_instance(graph);
	...
	for(int i = 0; i < MAX_CLQ_SIZE; i++){
		ans_clq.push_back(NEW_OLD[MaxCLQ_Stack[i]] - 1);
	}
	return ans_clq;
}
